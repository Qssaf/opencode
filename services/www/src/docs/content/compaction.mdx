...
cannot fit the selected model, OpenCode retries within the same prompt ceiling
with a shorter text version: old tool results and media are reduced first, then
older exchanges are omitted if needed. The original messages remain in session
history.
