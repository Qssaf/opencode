...
messages, and advertised tools. By default it starts compaction at 90% of the
...
compact when estimated tokens >= (buffer configured ? usable window - buffer : floor(90% of usable window))
```

For example, with a 200,000-token context and a 128,000-token input limit,
the default threshold is 115,200 tokens:
...
cannot fit the selected model, OpenCode retries within the same input threshold
with a shorter text version:
...
| `buffer` | unset | Replaces the 90% threshold with `usable window - buffer`. Set it to reserve a fixed number of tokens. |
...
| --- | --- |
| Threshold | OpenCode starts at 90% of the smaller context or input limit, unless `buffer` replaces it with a fixed reserve. |
